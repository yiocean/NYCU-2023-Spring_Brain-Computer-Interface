...
def begin_car(code, freq, savename='', trials=1):
    ...
    plt.plot(np.abs(fft_data)[int(len(fft_data)/1000*10):int(len(fft_data)/1000*20),])
    plt.savefig("REC/%s_%s_freq_%s.png" % (savename, time_str(), freq))
    peak = np.abs(fft_data)[int(len(fft_data)/1000*10):int(len(fft_data)/1000*20),]
    print("peak value:", np.max(peak))
    if np.max(peak) < 950000:
        pred = 0    # look nothing or 13 Hz
    else:
        pred = 1    # look 17 Hz
    print("pred: ", pred)
    return pred
...
if __name__ == "__main__":
    ...
    code =[]
    for i in range(2):
        code.append(begin_car(i, 17, i))
    
    if code == [0,1]:   # forward
        ser.write(b'1')  #forward
        ser.write(b'1')  #forward
    elif code == [1,0]:   # right
        ser.write(b'4') #right
    elif code == [1,1]:   # left
